...
try {
const r = await db.query(
INSERT INTO revisoes   (cidade, cliente_nome, cliente_telefone, cliente_cpf, modelo_moto, chassi_moto, km, tipo_revisao, observacao, status)   VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'ABERTA')   RETURNING *,
...
try {
const r = await db.query(
SELECT id, created_at, cidade, cliente_nome, modelo_moto, chassi_moto, km, tipo_revisao, total, status   FROM revisoes   ORDER BY created_at DESC
);
res.json(r.rows);
} catch (err) {
...
try {
let query =   SELECT id, nome, preco, estoque, cidade, tipo_moto   FROM pecas  ;
const params = [];
...
INSERT INTO pecas (nome, preco, estoque, cidade, tipo_moto)   SELECT nome, preco, $1, $2, tipo_moto   FROM pecas   WHERE id = $3,
...
try {
const result = await db.query(  SELECT    id,   modelo,   cor,   chassi,   filial,   status,   cnpj_empresa,   ano_moto,   valor_compra,   repasse,   CASE   WHEN santander = true THEN true   ELSE false   END AS santander   FROM motos   ORDER BY id  );
...
SELECT id, cliente_nome, cliente_telefone, forma_pagamento, observacao, chassi_moto, km,   total, cidade, created_at   FROM vendas   WHERE id = $1,
[id]
);
...
SELECT id, cliente_nome, total, created_at, cidade,forma_pagamento,observacao   FROM vendas   ORDER BY created_at DESC
);
...
const pendRes = await client.query(  
  ...
  RETURNING id`,  
  [  
    moto.id, moto.modelo, moto.cor, moto.chassi,  
    moto.filial, filial_venda,  
    nome_cliente, cpf || null, telefone, numero_cliente || null,  
    Number(valor), forma_pagamento || null, !!brinde, gasolina ? Number(gasolina) : null, como_chegou || null,  
    !!moto.santander, moto.cnpj_empresa || null, moto.valor_compra != null ? Number(moto.valor_compra) : null, moto.repasse != null ? Number(moto.repasse) : null  
  ]  
);  

// trava a moto até aprovar/recusar  
await client.query(  
  `UPDATE motos  
...
try {
const r = await db.query(
SELECT *   FROM vendas_motos_pendentes   WHERE status = 'PENDENTE'   ORDER BY created_at DESC
);
...
app.post("/vendas-motos-pendentes/:id/aprovar", async (req, res) => {
const { id } = req.params;
const { aprovado_por } = req.body; // pode passar username da diretoria
...
// confere se moto ainda está pendente  
const motoRes = await client.query(  
...
await client.query(  
  ...
  [  
    p.moto_id, p.modelo, p.cor, p.chassi, p.filial_origem, p.filial_venda,  
    p.nome_cliente, p.cpf, p.telefone, p.valor, p.forma_pagamento, p.brinde, p.gasolina,  
    p.como_chegou, p.santander, p.numero_cliente, p.valor_compra, p.repasse, p.cnpj_empresa  
  ]  
);  
...
// baixa capacete se brinde (AGORA É NO MOMENTO DA APROVAÇÃO)  
if (p.brinde === true) {  
...
await client.query(  
  `UPDATE motos SET status = 'DISPONIVEL' WHERE id = $1`,  
  [p.moto_id]  
);  
